...
const getPadAmount = (message: string): number => {
  const minPadAmount = 15;

  const messageLength = message.length;

  if (messageLength < minPadAmount) return minPadAmount;

  if (messageLength > MAX_MESSAGE_LENGTH) return MAX_MESSAGE_LENGTH;

  return messageLength;
};

const gherSay = (message: string) => {
  const hour = new Date().getHours();
  const greetings = ["Good morning!", "Good afternoon!", "Good evening!"];
  let greeting = greetings[0];
  if (hour > 12) greeting = greetings[1];
  if (hour > 18) greeting = greetings[2];
  ...
  messageParts.forEach((messagePart, index) => {
    console.log(
      ` ${
        index === 0 ? "_(()__" : index === 1 ? " ||   " : "     "
      } │ ${messagePart.padEnd(padAmount, " ")} │`
    );
  });

  console.log(
    `${messageParts.length < 2 ? "  ||    " : "        "}╰${"─".repeat(
      padAmount + 2
    )}╯`
  );
  console.log(" ");
